...
public class QemuCommandBuilder {
    ...
    /**
     * Automatically adds a -net user,<userNetworkStack.join(',')> option to the cmd.
     */
    private List<String> userNetworkStack;
    
    /**
     * If set adds a restrict=on to the user network stack, disallowing any network connections except specified forwardings.
     */
    ...
    /**
     * Redirect the serial interface /dev/ttyS0 to standard I/O (VERY useful for debugging etc.)
     * 
    ...
    public QemuCommandBuilder addMonitorPort(int port, boolean nowait) {
        addArgument("-monitor", String.format("tcp:127.0.0.1:%d,server" + (nowait ? ",nowait" : ""), port));
        return this;
    }
     ...
     * 
     * This opens a port on the host that is forwarded to the specified port on the guest.
     * 
     * @return
     */
    public QemuCommandBuilder addHostFwd(int hostport, int guestport) {
        ...
        userNetworkStack.add(String.format("hostfwd=tcp:127.0.0.1:%d-10.0.2.15:%d", hostport, guestport));
        return this;
    }
    ...
    public List<String> getCommandLine() {
        ...
        sb.append("user,");
        if (restrictNetwork) sb.append("restrict=on,");
